...
MACRO(CREATE_EXPORT_FILE VAR TARGET API_FUNCTIONS)
  IF(MSVC)
    # .def file name
    SET(DEF_FILE_BASE ${TARGET}_exports.def)
    # Create a .def generation helper for each configuration
    # (since LOCATION is config-dependent)
    foreach(CFG ${CMAKE_CONFIGURATION_TYPES})
      SET(INPUT_FILES "")
      SET(DEF_FILE "${CMAKE_CURRENT_BINARY_DIR}/${CFG}/${DEF_FILE_BASE}")
      foreach(LIB ${ARGN})
        get_target_property(LIB_FILE ${LIB} LOCATION_${CFG})
        SET(INPUT_FILES "${INPUT_FILES} ${LIB_FILE}")
      endforeach()
      file(MAKE_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/${CFG})
      CONFIGURE_FILE(
        ${MYSQL_CMAKE_SCRIPT_DIR}/make_def.cmake.in
        ${CMAKE_CURRENT_BINARY_DIR}/${CFG}/make_def_${TARGET}.cmake
        @ONLY)
    endforeach()
    # Add a custom command to generate the .def file once we have the .lib
    add_custom_command(TARGET ${TARGET}
                       PRE_LINK
                       COMMAND ${CMAKE_COMMAND} -P ${CMAKE_CURRENT_BINARY_DIR}/$<CONFIGURATION>/make_def_${TARGET}.cmake)
    set_target_properties(${TARGET} PROPERTIES LINK_FLAGS "/DEF:\$(Configuration)/${DEF_FILE_BASE}")
  elseif(WIN32)
...
MACRO(MERGE_STATIC_LIBS TARGET OUTPUT_NAME LIBS_TO_MERGE)
  ...
  SET(SOURCE_FILE ${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_depends.cxx)
...
function(COLLECT_DEPENDENCIES output_var)
  set(all_deps "")
  foreach(LIB ${ARGN})
    if(TARGET ${LIB})
      set(all_deps ${all_deps} ${LIB})
      get_target_property(lib_depends ${LIB} LINK_LIBRARIES)
      if(NOT "${lib_depends}" STREQUAL "lib_depends-NOTFOUND")
        COLLECT_DEPENDENCIES(extra_deps ${lib_depends})
        set(all_deps ${all_deps} ${extra_deps})
      endif()
    endif()
  endforeach()
...
MACRO(MERGE_LIBRARIES)
  ...
  list(APPEND LIBS ${ALL_LIBS})
  if(NOT ARG_STATIC AND NOT ARG_SHARED AND NOT ARG_MODULE)
  ...
  IF(ARG_STATIC)
    ...
    ENDIF()
    COLLECT_DEPENDENCIES(ALL_LIBS ${LIBS})
    MERGE_STATIC_LIBS(${TARGET} ${ARG_OUTPUT_NAME} "${ALL_LIBS}") 
  ELSEIF(ARG_SHARED OR ARG_MODULE)
    ...
    if(MSVC)
      SET(DUMMY ${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_dummy.c)
      CONFIGURE_FILE_CONTENT("" ${DUMMY})
      ADD_LIBRARY(${TARGET} ${LIBTYPE} ${DUMMY})
      CREATE_EXPORT_FILE(SRC ${TARGET} "${ARG_EXPORTS}" ${LIBS})
    else()
      CREATE_EXPORT_FILE(SRC ${TARGET} "${ARG_EXPORTS}" ${LIBS})
    ...
    if(MSVC)
      ...
      TARGET_LINK_LIBRARIES(${TARGET} PRIVATE "-Wl,--whole-archive" ${LIBS} "-Wl,--no-whole-archive")
    endif()
    IF(ARG_OUTPUT_NAME)
