...
Ray Tracing findet vor allem bei der Vorberechnung von Grafiken statt. Allerdings wird dort zunächst auch Rasterung eingesetzt,
...
können aber sehr viele Dreiecke in einer Sekunde gezeichnet werden. 
...
Diese Projektion wird nach der Vertexverarbeitung durchgeführt. Auf alle Vertices werden die gleichen
Berechnungen angewendet, diese werden in Hardware in der Regel parallel 
auf mehreren Recheneinheiten ausgeführt. Die Recheneinheiten rechnen im Allgemeinen mit Vektoren.
...
auf eineer Grafikkarte ohne Geometrieeinheit sind alle Verarbeitungsschurtte "`statisch"' bezüglich der Vertices -- nur existierende
Werte können manipuliert werden. Gleiches gilt für Dreiecksdaten. Das \emph{Erzeugen} von Vertices kann auf diesen Grafikkarten
also nur durch die CPU erfolgen, woraufhin die Daten zur GPU übertragen werden.
Im Schritt der Geometrieverarbeitung kann aber die GPU selbst Vertices und Dreiecke erzeugen
...
in der diese Operation ausgeführt wird, zuordnen: mit \emph{Vertexfrequenz} (per Vertex) berechnete Operationen werden dem Vertexprogramm zugeordnet.
...
\paragraph{Operanden:} Eine Operation muss mindestens in derjenigen Frequenz ausgeführt werden, welche dem Maximum der Operanden-Frequenzen
entspricht. So muss z.B. eine Addition von zwei Werten per Fragment ausgeführt werden, wenn einer der Summanden per Fragment berechnet wurde.
...
dass durch weitere Optimierungsschritte wie Dead Code Elimination einige Operationen auf uniformen Werten entfernt werden.
...
sondern eine \emph{quadratische} Funktion. Da per Mesh gegebene Operanden praktisch als
