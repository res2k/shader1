...
Der zweite Ausgabeparameter nimmt die auszugebende Pixelfarbe entgegen.
...
\hspace*{1cm}\Gnt{BEZEICHNER}{\scriptsize (\ref{BEZEICHNER})}\Gspace\Gopt{\Gnt{attribut\_liste\_oder\_array\_element}{\scriptsize (\ref{attribut_liste_oder_array_element})} }\Gspace\Gt{=}\Gspace\Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} \\
...
einer "`rechten Seite"', dem direkt folgenden, zuzuweisenden \Gnt{ausdruck}{\scriptsize (\ref{ausdruck})}, zusammen.
...
\hspace*{1cm}\Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} \Gspace\Gt{?}\Gspace\Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} \Gspace\Gt{:}\Gspace\Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} \\

Ein ternärer Ausdruck setzt sich aus einem \Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} (der "`Bedingung"') sowie zwei weiteren
...
\hspace*{1cm}\Gnt{typ\_array}{\scriptsize (\ref{typ_array})} \\
...
insbesondere ist der Basistyp nicht auf \Gnt{typ\_basis}{\scriptsize (\ref{typ_basis})} beschränkt sondern kann selbst ein Arraytyp sein).
...
"`echte"' Blöcke (und nicht, wie sonst üblich, eine einzelne Anweisung \Gnt{kommando}{\scriptsize (\ref{kommando})} die eine Blockanweisung sein kann).
...
\hspace*{1cm}\Gt{for}\Gspace\Gt{(}\Gspace\Gnt{schleife\_for\_kopf}{\scriptsize (\ref{schleife_for_kopf})} \Gspace\Gt{)}\Gspace\Gt{\{}\Gspace\Gnt{block}{\scriptsize (\ref{block})} \Gspace\Gt{\}}\\
\Gnt{schleife\_for\_kopf}:\label{schleife_for_kopf}\\
\hspace*{1cm}\Gopt{\Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} }\Gspace\Gt{;}\Gspace\Gopt{\Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} }\Gspace\Gt{;}\Gspace\Gopt{\Gnt{ausdruck}{\scriptsize (\ref{ausdruck})} }\\
